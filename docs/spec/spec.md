...
- When non-deadline Koios calls fail after retries, each scan/rollback iteration falls back to Blockfrost individual-request endpoints (~9 RPS). Blockfrost requests have a 20-second `AbortSignal` timeout. Deadline exhaustion remains a clean stop signal instead of triggering fallback work. Fallback failures for reference scripts and metadata are logged as NOTIFY.
