...
| Script catalog and datum utilities | `GET /scripts`, `POST /datum` | `controllers/scripts.controller.ts`, `controllers/datum.controller.ts`, `config/scripts.ts` | Builders can fetch canonical scripts and transform CBOR/JSON payloads for integration workflows. Canonical deployed contract names follow the shared slug rule `<app><[ord|mnt|ref|roy]><[mpt]>`, while `/scripts` still serves legacy type aliases during migration. |
