...
| Script catalog and datum utilities | `GET /scripts`, `POST /datum` | `controllers/scripts.controller.ts`, `controllers/datum.controller.ts`, `services/scripts.service.ts` | Builders can fetch canonical scripts and transform CBOR/JSON payloads for integration workflows. Canonical script handles follow `<slug><ordinal>@handlecontract`, `/scripts` marks the highest ordinal per public type as `latest`, and it hydrates `unoptimizedCbor` from the owning repo’s `<network>/<slug>.unoptimized.cbor` artifact when present. |
