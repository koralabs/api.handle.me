...
| Script, datum, and policy utilities | `GET /scripts`, `POST /datum`, `GET /policies` | `routes/policies.route.ts`, `controllers/scripts.controller.ts`, `controllers/datum.controller.ts`, `controllers/policies.controller.ts`, `services/scripts.service.ts` | Builders can fetch canonical scripts, transform CBOR/JSON payloads, and retrieve normalized Handle policy settings. Canonical script handles follow `<slug><ordinal>@handlecontract`, `/scripts` marks the highest ordinal per public type as `latest`, and it hydrates `unoptimizedCbor` from the local build-bundled `config/script-artifacts/<network>/<slug>.unoptimized.cbor` artifact when present. |
