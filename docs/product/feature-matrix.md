...
| Script catalog and datum utilities | `GET /scripts`, `POST /datum` | `controllers/scripts.controller.ts`, `controllers/datum.controller.ts`, `services/scripts.service.ts` | Builders can fetch canonical scripts and transform CBOR/JSON payloads for integration workflows. |
